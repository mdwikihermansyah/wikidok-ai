...
# * Load Cohere API key from environment variable if available
...
if "cohere_api_key" not in st.session_state:
    st.session_state.cohere_api_key = cohere_api_key_env
...
if "retriever_type" not in st.session_state:
    st.session_state.retriever_type = "Contextual compression"  # Default tanpa Cohere
...
def expander_model_parameters(
    ...
    if LLM_provider == "OpenAI":
        # * Use environment variable if available, otherwise allow user input
...
def sidebar_and_documentChooser():
    ...
    with st.sidebar:
        ...
        if st.session_state.retriever_type == list_retriever_types[0]:  # Cohere
            ...
            st.session_state.cohere_api_key = st.text_input(
                ...
                type="password",
                placeholder="insert your API key",
                value=cohere_api_key_env if cohere_api_key_env else "",
            )
    ...
    with tab_open_vectorstore:
        ...
        st.write("Please enter the path to your Vectorstore:")
        
        # Use text input instead of tkinter file dialog
        vectorstore_path = st.text_input(
            "Vectorstore Path",
            placeholder="Enter the full path to your vectorstore directory",
            help="Example: /path/to/your/vectorstore"
        )
        ...
        if clicked and vectorstore_path:
            # Check inputs
            error_messages = []
            if (
            ...
            # if API keys are inserted, start loading Chroma index, then create retriever and ConversationalRetrievalChain
                ...
                selected_vectorstore_path = vectorstore_path

                if not selected_vectorstore_path or selected_vectorstore_path.strip() == "":
...
def create_retriever(
    ...
    Parameters:
        ...
        cohere_model (str): model used by Cohere, in ["rerank-v2.0"]
...
def CohereRerank_retriever(
    base_retriever, cohere_api_key, cohere_model="rerank-v2.0", top_n=10
):
    ...
    Parameters:
       ...
       cohere_model: the Cohere model, default = "rerank-v2.0"
    ...
    try:
        ...
        st.warning(f"⚠️ Cohere reranker error: {str(e)}. Falling back to base retriever.")
        return base_retriever
